...
import (
	...
	"github.com/webitel/im-account-service/internal/service/auth"
...
func NewAccountService(manager *service.Manager) *AccountService {
	return &AccountService{Service: manager}
}
...
// func (*AccountService) mustEmbedUnimplementedAccountServer() {}
	...
	case *v1.TokenRequest_Identity:
		{
			...
			return &v1.Authorization{
				...
				Token: &v1.AccessToken{
					TokenType:    session.Grant.Type,
					AccessToken:  (auth.SessionTokenPrefix + session.Grant.Token),
					RefreshToken: session.Grant.Refresh,
...
func (api *AccountService) Logout(ctx context.Context, req *v1.LogoutRequest) (*v1.LogoutResponse, error) {
	...
	// Authorization
	rpc, err := api.Service.GetContext(
		// RPC Operation Context
		ctx, service.Authenticate{
			Require: true,
			Schemes: []service.AuthenticationScheme{
				auth.TokenAuthentication{},
				auth.JWTokenAuthentication{},
				auth.WebitelAuthentication{
					Client: api.Service.Options().Webitel,
				},
			},
		},
	)
	...
	// Remove (cached) session Authorization ..
	err = rpc.Service.DelSession(
		rpc.Context, rpc.Session,
	)
	if err != nil {
		// something went wrong
		return nil, err
...
func currentAuthorizationProtoV1(rpc *service.Context) (*v1.Authorization, error) {
...
func (api *AccountService) GrantTokenForUserIdentity(ctx context.Context, req *v1.TokenRequest) (*service.Context, error) {

	idtoken := req.GetIdentity()
	if idtoken == nil {
		return nil, errors.BadRequest(
			errors.Message("identity: required but missing"),
		)
	}
	// [Verify]:
	...
	rpc, err := api.Service.GetContext(
		...
		//
		// Used as a [hint] to identify previous (device) session
	...
	err = rpc.App.Clients().NewContact(contact)
	if err != nil {
		return rpc, err
	}
	...
	err = api.Service.AddContact(rpc.Context, contact)
	if err != nil {
