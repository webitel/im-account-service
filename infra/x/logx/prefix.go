...
func ModuleLogger(module string, logger *slog.Logger) *slog.Logger {
  return slog.New(
    ModuleHandler(module, logger.Handler()),
  )
}

func ModuleHandler(module string, handler slog.Handler) slog.Handler {
  if module == "" {
    return handler
  }
  module = "[ " + module + " ] "
  // is already desired handler ?
  impl, _ := handler.(prefixHandler)
  if impl.prefix == module {
    // duplicate: return original
    return handler
  }
  return prefixHandler{prefix: module, Handler: handler}
}
...
func (h prefixHandler) Handle(ctx context.Context, event slog.Record) error {
...
func (h prefixHandler) WithGroup(name string) slog.Handler {
	return prefixHandler{prefix: name, Handler: h.Handler.WithGroup(name)}
}
